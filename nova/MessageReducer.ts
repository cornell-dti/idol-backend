...
export const reduceMessage = async function (
  req: Request,
  res: Response
): Promise<MemberResponse | ImageResponse | ErrorResponse> {
  if (!req.body.member) {
    return {
      status: 400,
  ...
  if (req.body.member) {
    const response = await updateMember(req.body.member);
    return response;
  }
};

let updateMember = async function (
  member: Member
  ...
  fs.writeFile(
    getFilePath(member.email),
    JSON.stringify(req.body.member),
    function (err) {
      if (err) {
        return {
          status: 500,
